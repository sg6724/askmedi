...
class FakeReportRepository implements ReportRepository {
  ...
  @override
  Future<ReportSummary> confirm(String reportId, List<ReportValue> values,
    ...
    return ReportSummary.fromJson({
      ...
      'values': [for (final v in values) {...v.toJson(), 'status': 'low'}],
      'sources': [
        {'title': 'MedlinePlus: Hemoglobin test', 'url': 'https://medlineplus.gov/hb'},
      ],
...
void main() {
  ...
  testWidgets('pick -> edit extracted value -> confirm -> Summary / Key values / Chart',
      (tester) async {
    final repo = FakeReportRepository();
    await pumpRouted(tester, const ReportScreen(), overrides: [
  ...
  testWidgets('file too large (413) shows an error and stays on the picker',
    ...
    await pumpRouted(tester, const ReportScreen(), overrides: [
      ...
      reportRepositoryProvider.overrideWithValue(FakeReportRepository(
          parseError: const ApiException(statusCode: 413, code: 'file_too_large'))),
    ]);
