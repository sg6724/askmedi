      ...
      'hospitals': [
        {
          ...
          'emergency': true,
          'maps_url': 'https://www.google.com/maps/search/?api=1&query=18.53,73.87',
        },
        {
          'name': 'Small Clinic',
          'lat': 18.54,
          ...
          'phone': null,
          'emergency': null,
          'maps_url': 'https://www.google.com/maps/search/?api=1&query=18.54,73.88',
        },
      ],
    });
  }
...
void main() {
  ...
  testWidgets('PIN code search lists hospitals; call and directions open links',
      (tester) async {
    final repo = FakeHospitals();
    final opener = RecordingOpener();
    await pumpRouted(tester, const HospitalsScreen(),
        opener: opener, overrides: [hospitalsRepositoryProvider.overrideWithValue(repo)]);
  ...
  testWidgets('unknown place shows location_not_found message', (tester) async {
    await pumpRouted(tester, const HospitalsScreen(), overrides: [
      hospitalsRepositoryProvider.overrideWithValue(FakeHospitals(
          error: const ApiException(statusCode: 404, code: 'location_not_found'))),
    ]);
    ...
    expect(find.text("We couldn't find that place. Check the PIN code or area."),
        findsOneWidget);
  });
}
